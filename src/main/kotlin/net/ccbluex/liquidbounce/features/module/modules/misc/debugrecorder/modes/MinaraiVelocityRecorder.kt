 ...
 */
object MinaraiVelocityRecorder : ModuleDebugRecorder.DebugRecorderMode<TrainingDataMinarai>("MinaraiVelocityRecorder") {
    private var targetTracker =
        tree(
            TargetTracker(
    ...
    private val trainingCollection = Int2ObjectOpenHashMap<MutableList<TrainingDataMinarai>>()
    ...
    private inline fun <V> Int2ObjectOpenHashMap<V>.getOrPut(
        key: Int,
        valueProvider: () -> V,
    ): V =
        get(key) ?: valueProvider().also {
            put(key, it)
        }
    ...
    @Suppress("unused")
    private val attackHandler =
        sequenceHandler<AttackEntityEvent> { event ->
            ...
            var inactivity = 0
            var buffer: MutableList<TrainingDataMinarai>? = null
            waitUntil {
                ...
                val raytraceTarget =
                    raytraceEntity(distance, rotation) { e ->
                        e == entity
                    }
